...
def update_image(root, image_label):
    ...
    global Coords
    image_raw = cv.imread(function_that_yields_new_image_paths(), cv.IMREAD_COLOR)
    im_shp = image_raw.shape
    im_temp1 = cv.resize(image_raw, (im_shp[1] // resize, im_shp[0] // resize)) 
    im_temp2 = cv.rectangle(im_temp1, Coords["start"], Coords["end"], (0,0,255), 1) if Coords["n_pnts"] == 2 else im_temp1
    tk_image = ImageTk.PhotoImage(image = Image.fromarray(im_temp2))
    image_label.configure(image=tk_image)
    image_label.image = tk_image
    root.after(300, update_image, root, image_label)
...
def getcoord(event, num_clicks):
    ...
    global Coords
    if Coords["n_pnts"] < 2:
        index_use = Coords["n_pnts"]
        Coords["x"][index_use] = event.x 
        Coords["y"][index_use] = event.y 
        Coords["n_pnts"] = index_use + 1
        if index_use == 1:
            Coords["start"] = [min(Coords["x"]), min(Coords["y"])]
            Coords["end"] = [max(Coords["x"]), max(Coords["y"])]
    else:
        Coords["n_pnts"] = 0 # remove last 2 points
